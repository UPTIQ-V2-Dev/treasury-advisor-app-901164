...
const getRecommendationsByClientTool: MCPTool = {
...
const generateRecommendationsTool: MCPTool = {
...
const getRecommendationByIdTool: MCPTool = {
...
const approveRecommendationTool: MCPTool = {
    ...
    fn: async (inputs: { recommendationId: string; reviewerId: string; comments?: string }) => {
        await recommendationService.approveRecommendation(inputs.recommendationId, inputs.reviewerId, inputs.comments);
        return { success: true };
    }
};

const rejectRecommendationTool: MCPTool = {
    ...
    fn: async (inputs: { recommendationId: string; reviewerId: string; reason: string }) => {
        await recommendationService.rejectRecommendation(inputs.recommendationId, inputs.reviewerId, inputs.reason);
        return { success: true };
    }
};

const getRecommendationSummaryTool: MCPTool = {
...
const updateRecommendationPriorityTool: MCPTool = {
    ...
    fn: async (inputs: { recommendationId: string; priority: 'HIGH' | 'MEDIUM' | 'LOW' }) => {
        await recommendationService.updateRecommendationPriority(inputs.recommendationId, inputs.priority);
        return { success: true };
    }
};

const implementRecommendationTool: MCPTool = {
    ...
    fn: async (inputs: { recommendationId: string; implementationDate: string; notes?: string }) => {
        await recommendationService.implementRecommendation(
            inputs.recommendationId,
            inputs.implementationDate,
            inputs.notes
        );
        return { success: true };
    }
...
const exportRecommendationsTool: MCPTool = {
...
const analyzeRecommendationOpportunityTool: MCPTool = {
    id: 'recommendation_analyze_opportunity',
    name: 'Analyze Recommendation Opportunity',
    description:
        'Analyze potential recommendation opportunities for a client without generating formal recommendations',
    inputSchema: z.object({
        clientId: z.string().uuid()
    }),
    outputSchema: z.object({
...
const getRecommendationMetricsTool: MCPTool = {
    ...
    fn: async (inputs: { clientId: string; includeImplementedSavings?: boolean }) => {
        ...
        const categoryData: { [key: string]: { count: number; totalSavings: number } } = {};
        recommendations.forEach(r => {
            const category = (r as any).product?.category || 'unknown';
            const benefit = r.estimatedBenefit as any;
...
export const recommendationTools: MCPTool[] = [
