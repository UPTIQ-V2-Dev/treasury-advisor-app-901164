...
const getSystemHealthTool: MCPTool = {
...
const getSystemLogsTool: MCPTool = {
...
const createMaintenanceTaskTool: MCPTool = {
...
const getUsageAnalyticsTool: MCPTool = {
    ...
    fn: async (inputs: { period?: string; granularity?: string }) => {
        const analytics = await adminService.getSystemUsageAnalytics(inputs.period);
        return analytics;
    }
};

const getSystemMetricsTool: MCPTool = {
...
const getServiceStatusTool: MCPTool = {
    ...
    fn: async (inputs: { service?: string }) => {
        const health = await adminService.getSystemHealth();
        const timestamp = new Date().toISOString();

        if (inputs.service) {
            const serviceStatus = health.services[inputs.service as keyof typeof health.services];
            return {
                services: {
                    [inputs.service]: {
...
const getSystemStatsTool: MCPTool = {
