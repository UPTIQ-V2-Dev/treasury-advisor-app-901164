 ...
 * @returns {Promise<BankConnection[]>}
 */
const queryBankConnections = async (
    filter: any,
    options: {
        limit?: number;
        page?: number;
...
): Promise<{ connections: BankConnection[]; total: number; pages: number }> => {
 ...
 */
const deleteBankConnectionById = async (connectionId: string): Promise<void> => {
    const connection = await getBankConnectionById(connectionId);
    if (!connection) {
        throw new ApiError(httpStatus.NOT_FOUND, 'Bank connection not found');
    }
 ...
 */
const syncBankConnection = async (connectionId: string): Promise<ProcessingTask> => {
    const connection = await getBankConnectionById(connectionId);
    if (!connection) {
        throw new ApiError(httpStatus.NOT_FOUND, 'Bank connection not found');
    }
...
const getConnectionStats = async (
    ...
    lastSyncDate?: Date;
    syncFrequency: { [key: string]: number };
}> => {
    ...
    const stats = {
        ...
        syncFrequency: connections.reduce(
            (freq, conn) => {
