...
import { Client, ClientAccount, Prisma } from '../generated/prisma/index.js';
 ...
 * @returns {Promise<Client>}
 */
const updateClientById = async (clientId: string, updateBody: Prisma.ClientUpdateInput): Promise<Client | null> => {
 ...
 * @returns {Promise<Client[]>}
 */
const getClientsByRM = async (rmId: number): Promise<Client[]> => {
    return await prisma.client.findMany({
 ...
 */
const updateClientPreferences = async (clientId: string, preferences: any): Promise<void> => {
    const client = await getClientById(clientId);
    if (!client) {
        throw new ApiError(httpStatus.NOT_FOUND, 'Client not found');
    }
 ...
 */
const getClientAccounts = async (clientId: string): Promise<ClientAccount[]> => {
    const client = await getClientById(clientId);
    if (!client) {
        throw new ApiError(httpStatus.NOT_FOUND, 'Client not found');
    }
...
const updateClientAccount = async (
    clientId: string,
    accountId: string,
    updateData: Prisma.ClientAccountUpdateInput
