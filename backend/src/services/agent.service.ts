...
import { AgentAnalysis, AgentTask, AgentTaskStatus } from '../generated/prisma/index.js';
...
const triggerAgent = async (
    ...
    setTimeout(async () => {
        ...
        setTimeout(async () => {
            ...
            await updateAgentTaskProgress(
                agentTask.id,
                AgentTaskStatus.COMPLETED,
                100,
                [`${agentType} agent processing completed successfully`],
                results
            );
 ...
 * @returns {Promise<AgentTask>}
 */
const getAgentTask = async (agentTaskId: string): Promise<AgentTask> => {
 ...
 * @returns {Promise<AgentAnalysis>}
 */
const getAgentAnalysis = async (analysisTaskId: string): Promise<AgentAnalysis> => {
 ...
 * @returns {Promise<AgentTask>}
 */
const updateAgentTaskProgress = async (
    agentTaskId: string,
 ...
 * @returns {Promise<AgentAnalysis>}
 */
const updateAgentAnalysis = async (analysisId: string, status: AgentTaskStatus): Promise<AgentAnalysis> => {
 ...
 */
const generateMockResults = (agentType: string): object => {
    switch (agentType) {
        case 'recommendation':
            return {
                recommendations: [
 ...
 * @returns {Promise<object>}
 */
const generateMockAnalysisResults = (analysisType: string, fileCount: number): object => {
    const baseResults = {
