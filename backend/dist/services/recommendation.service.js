 ...
 */
const generateRecommendations = async (clientId) => {
 ...
 */
const processRecommendationGeneration = async (clientId, taskId) => {
    // Update task status
    await prisma.processingTask.update({
        where: { taskId },
    ...
    await prisma.processingTask.update({
        ...
        data: {
            ...
            results: {
                ...
                totalEstimatedBenefit: recommendations.reduce((sum, r) => sum + (r.estimatedBenefit.annualSavings || 0), 0)
            }
        }
    });
};
 ...
 */
const generateAIRecommendations = (client, products) => {
    const recommendations = [];
    // Calculate client metrics
    const totalBalance = client.ClientAccount.reduce((sum, acc) => sum + (acc.balance || 0), 0);
    ...
    if (client.riskProfile === 'medium' || client.riskProfile === 'low') {
        ...
        if (investmentProduct && totalBalance > 250000) {
            recommendations.push({
                ...
                priority: client.riskProfile === 'low' ? 'LOW' : 'MEDIUM',
                rationale: {
        ...
        if (creditProduct) {
            recommendations.push({
                ...
                rationale: {
                    ...
                    analysis: 'Client shows seasonal cash flow variations. A revolving credit facility could smooth cash management and reduce the need for excess cash reserves.',
                    keyFactors: [
 ...
 */
const calculateAverageMonthlyFlow = (transactions) => {
 ...
 */
const hasSeasonalCashFlow = (transactions) => {
 ...
 */
const getRecommendationsByClientId = async (clientId) => {
 ...
 */
const provideFeedback = async (recommendationId, feedbackData) => {
    const recommendation = await prisma.recommendation.findUnique({
        where: { id: recommendationId }
    });
    if (!recommendation) {
 ...
 * @returns {Promise<void>}
 */
const approveRecommendation = async (recommendationId, reviewerId, comments) => {
    const recommendation = await prisma.recommendation.findUnique({
        where: { id: recommendationId }
    });
    if (!recommendation) {
 ...
 * @returns {Promise<void>}
 */
const rejectRecommendation = async (recommendationId, reviewerId, reason) => {
    const recommendation = await prisma.recommendation.findUnique({
        where: { id: recommendationId }
    });
    if (!recommendation) {
 ...
 */
const getRecommendationSummary = async (clientId) => {
    const client = await prisma.client.findUnique({ where: { id: clientId } });
    if (!client) {
        throw new ApiError(httpStatus.NOT_FOUND, 'Client not found');
    }
 ...
 * @returns {Promise<void>}
 */
const updateRecommendationPriority = async (recommendationId, priority) => {
    const recommendation = await prisma.recommendation.findUnique({
        where: { id: recommendationId }
    });
    if (!recommendation) {
 ...
 * @returns {Promise<void>}
 */
const implementRecommendation = async (recommendationId, implementationDate, notes) => {
    const recommendation = await prisma.recommendation.findUnique({
        where: { id: recommendationId }
    });
    if (!recommendation) {
 ...
 * @returns {Promise<any>}
 */
const exportRecommendations = async (clientId, format) => {
    const client = await prisma.client.findUnique({
        where: { id: clientId },
 ...
 * @returns {Promise<Recommendation[]>}
 */
const getHistoricalRecommendations = async (clientId) => {
    const client = await prisma.client.findUnique({ where: { id: clientId } });
    if (!client) {
        throw new ApiError(httpStatus.NOT_FOUND, 'Client not found');
    }
