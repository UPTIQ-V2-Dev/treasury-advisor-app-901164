 ...
 */
const startProcessingTask = async (clientId, type, options = {}) => {
 ...
 */
const getProcessingTaskById = async (taskId) => {
 ...
 * @returns {Promise<ProcessingTask[]>}
 */
const getClientProcessingTasks = async (clientId) => {
    // Validate client exists
    const client = await prisma.client.findUnique({
        where: { id: clientId }
    });
 ...
 */
const cancelProcessingTask = async (taskId) => {
 ...
 * @returns {Promise<{taskId: string}>}
 */
const retryProcessingTask = async (taskId) => {
 ...
 */
const getClientProcessingHistory = async (clientId, options = {}) => {
    // Validate client exists
    const client = await prisma.client.findUnique({
        where: { id: clientId }
    });
 ...
 */
const getProcessingLogs = async (taskId, level) => {
    const task = await getProcessingTaskById(taskId);
    if (!task) {
        throw new ApiError(httpStatus.NOT_FOUND, 'Task not found');
    }
 ...
 */
const updateTaskProgress = async (taskId, progress, currentStep) => {
    await prisma.processingTask.update({
        where: { taskId },
        data: {
            progress,
 ...
 */
const completeTask = async (taskId, results) => {
    await prisma.processingTask.update({
        where: { taskId },
        data: {
            status: TaskStatus.COMPLETED,
 ...
 * @param {Object} error
 */
const failTask = async (taskId, error) => {
    await prisma.processingTask.update({
        where: { taskId },
        data: {
 ...
 */
const addTaskLog = (taskId, message) => {
    if (!taskLogs[taskId]) {
        taskLogs[taskId] = [];
    }
 ...
 */
const emitTaskUpdate = (taskId, status, progress, currentStep) => {
 ...
 * @param {string} taskId
 */
const processTaskInBackground = async (taskId) => {
    try {
        const task = await getProcessingTaskById(taskId);
        if (!task)
            return;
 ...
 * @returns {Promise<Object>}
 */
const generateTaskResults = (task) => {
    const baseResults = {
