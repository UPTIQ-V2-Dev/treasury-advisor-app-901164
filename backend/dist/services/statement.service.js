...
import { StatementStatus, TaskStatus, TaskType } from '../generated/prisma/index.js';
 ...
 * @returns {Promise<Statement[]>}
 */
const getStatementsByClientId = async (clientId) => {
 ...
 */
const validateStatement = async (statementId) => {
    const statement = await getStatementById(statementId);
    if (!statement) {
        throw new ApiError(httpStatus.NOT_FOUND, 'Statement not found');
    }
 ...
 * @returns {Promise<Object>}
 */
const getStatementStatus = async (statementId) => {
    const statement = await getStatementById(statementId);
    if (!statement) {
        throw new ApiError(httpStatus.NOT_FOUND, 'Statement not found');
    }
 ...
 */
const startStatementParsing = async (fileIds) => {
 ...
 * @returns {Promise<Object[]>}
 */
const getUploadProgress = async (clientId) => {
    const statements = await prisma.statement.findMany({
        where: { clientId },
        include: {
            ProcessingTask: {
 ...
 * @returns {Promise<Object[]>}
 */
const validateBulkStatements = async (fileIds, validationRules) => {
    // Validate all file IDs exist
    const statements = await prisma.statement.findMany({
        where: { id: { in: fileIds } },
        ...
        const result = {
            ...
            metadata: {
                ...
                dateRange: statement.period
                    ? {
                        start: statement.period?.startDate || '2024-01-01',
                        end: statement.period?.endDate || '2024-01-31'
                    }
                    : undefined,
 ...
 * @returns {Promise<Object>}
 */
const getProcessingQueue = async (statusFilter) => {
    let whereClause = {
        status: {
 ...
 * @returns {Promise<ProcessingTask[]>}
 */
const reprocessStatements = async (fileIds, options) => {
    // Validate all file IDs exist
    const statements = await prisma.statement.findMany({
        where: { id: { in: fileIds } },
        include: { client: true }
    ...
    const eligibleStatuses = [StatementStatus.FAILED, StatementStatus.COMPLETED];
    if (options?.forceReprocess) {
        eligibleStatuses.push(StatementStatus.PROCESSING, StatementStatus.VALIDATED, StatementStatus.UPLOADED);
    }
    const ineligibleStatements = statements.filter(statement => !eligibleStatuses.includes(statement.status));
    if (ineligibleStatements.length > 0 && !options?.forceReprocess) {
        throw new ApiError(httpStatus.CONFLICT, `Files not eligible for reprocessing: ${ineligibleStatements.map(s => s.fileName).join(', ')}`);
    }
    const taskIds = [];
 ...
 * @returns {Promise<Object>}
 */
const getDataQuality = async (fileId) => {
    const statement = await getStatementById(fileId);
    if (!statement) {
        throw new ApiError(httpStatus.NOT_FOUND, 'Statement not found');
    }
    // Check if statement has been processed
    const processedStatuses = [StatementStatus.COMPLETED, StatementStatus.FAILED];
    if (!processedStatuses.includes(statement.status)) {
