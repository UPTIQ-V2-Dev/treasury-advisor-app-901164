...
const getAnalyticsOverviewTool = {
    ...
    description: 'Get comprehensive financial analytics overview for a client including cash flow, liquidity ratios, and key metrics',
...
const getCashFlowAnalyticsTool = {
...
const getCategoryAnalyticsTool = {
...
const getLiquidityAnalyticsTool = {
...
const getTrendAnalyticsTool = {
    id: 'analytics_get_trends',
    name: 'Get Trend Analytics',
    description: 'Get time-series trend analysis for specific financial metrics (inflow, outflow, balance, transactions)',
    inputSchema: z.object({
        clientId: z.string().uuid(),
...
const getAnalyticsSummaryTool = {
    id: 'analytics_get_summary',
    name: 'Get Analytics Summary',
    description: 'Get complete analytics summary including all key insights: overview, cash flow, categories, liquidity, patterns, and trends',
    inputSchema: z.object({
        clientId: z.string().uuid(),
        startDate: z.string().optional(),
        endDate: z.string().optional()
...
const exportAnalyticsDataTool = {
    id: 'analytics_export_data',
    name: 'Export Analytics Data',
    description: 'Export comprehensive analytics data in various formats (CSV, PDF, Excel, JSON) for reporting and external analysis',
    inputSchema: z.object({
        clientId: z.string().uuid(),
...
const getFinancialHealthScoreTool = {
    id: 'analytics_get_health_score',
    name: 'Get Financial Health Score',
    description: 'Calculate overall financial health score based on liquidity, cash flow patterns, and spending efficiency',
    inputSchema: z.object({
        clientId: z.string().uuid()
    }),
    outputSchema: z.object({
    ...
    fn: async (inputs) => {
        ...
        const cashFlowScore = Math.min(100, Math.max(0, 50 + (overview.netCashFlow / Math.max(1, overview.totalOutflow)) * 50));
        const efficiencyScore = Math.min(100, Math.max(0, 100 - liquidity.volatility * 100 // Lower volatility = higher efficiency
        ));
...
const getComparativeAnalyticsTool = {
    ...
    fn: async (inputs) => {
        ...
        const inflowPercent = comparisonPeriod.totalInflow > 0 ? (inflowChange / comparisonPeriod.totalInflow) * 100 : 0;
        const outflowPercent = comparisonPeriod.totalOutflow > 0 ? (outflowChange / comparisonPeriod.totalOutflow) * 100 : 0;
        const netCashFlowPercent = Math.abs(comparisonPeriod.netCashFlow) > 0
...
const getForecastingAnalyticsTool = {
    id: 'analytics_get_forecasting',
    name: 'Get Cash Flow Forecasting Analytics',
    description: 'Get predictive cash flow analysis with seasonality patterns and strategic recommendations based on historical data',
    inputSchema: z.object({
        clientId: z.string().uuid(),
        forecastPeriod: z
            .string()
    ...
    fn: async (inputs) => {
        const forecasting = await analyticsService.getForecastingAnalytics(inputs.clientId, inputs.forecastPeriod || '90d', inputs.confidenceLevel || 0.85);
        return forecasting;
    }
};
// Benchmarking Analytics Tool
const getBenchmarkingAnalyticsTool = {
    id: 'analytics_get_benchmarking',
    name: 'Get Industry Benchmarking Analytics',
    description: 'Compare client financial metrics against industry standards with percentile ranking and performance analysis',
    inputSchema: z.object({
        clientId: z.string().uuid(),
    ...
    outputSchema: z.object({
        ...
        comparisonAreas: z.array(z.object({
            metric: z.string(),
    ...
    fn: async (inputs) => {
        const benchmarking = await analyticsService.getBenchmarkingAnalytics(inputs.clientId, inputs.industry, inputs.businessSegment);
        return benchmarking;
    }
};
// Enhanced Export Analytics Tool
const exportEnhancedAnalyticsTool = {
    id: 'analytics_export_enhanced',
    name: 'Export Enhanced Analytics Data',
    description: 'Export comprehensive analytics with professional templates, enhanced formatting, and customizable sections',
    inputSchema: z.object({
        clientId: z.string().uuid(),
        format: z.enum(['csv', 'pdf', 'excel', 'json']),
    ...
    fn: async (inputs) => {
        const exportData = await analyticsService.exportEnhancedAnalytics(inputs.clientId, inputs.format, inputs.template, inputs.startDate, inputs.endDate, inputs.sections);
        return exportData;
    }
};
// Cash Flow Forecasting Tool (Simplified for quick access)
const quickForecastTool = {
    ...
    outputSchema: z.object({
        thirtyDayForecast: z.array(z.object({
            date: z.string(),
            predictedBalance: z.number(),
            confidence: z.number()
        })),
    ...
    fn: async (inputs) => {
        ...
        if (fullForecast.forecast.length > 0) {
            const avgBalance = fullForecast.forecast.reduce((sum, day) => sum + day.predictedBalance, 0) /
