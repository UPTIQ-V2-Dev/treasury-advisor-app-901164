...
import { taskEmitter } from "../services/processing.service.js";
 ...
 */
const getTaskById = catchAsyncWithAuth(async (req, res) => {
 ...
 */
const getClientTasks = catchAsyncWithAuth(async (req, res) => {
 ...
 */
const startProcessing = catchAsyncWithAuth(async (req, res) => {
 ...
 */
const cancelTask = catchAsyncWithAuth(async (req, res) => {
    const { taskId } = req.params;
 ...
 */
const retryTask = catchAsyncWithAuth(async (req, res) => {
    const { taskId } = req.params;
 ...
 */
const getClientHistory = catchAsyncWithAuth(async (req, res) => {
    const { clientId } = req.params;
 ...
 */
const getTaskLogs = catchAsyncWithAuth(async (req, res) => {
    const { taskId } = req.params;
    const { level } = req.validatedQuery;
    const logs = await processingService.getProcessingLogs(taskId, level);
    res.json(logs);
});
/**
 * Server-sent events stream for real-time processing updates
 */
const getTaskStream = catchAsyncWithAuth(async (req, res) => {
    const { taskId } = req.params;
    // Verify task exists
    const task = await processingService.getProcessingTaskById(taskId);
    if (!task) {
