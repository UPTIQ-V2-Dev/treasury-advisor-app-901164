...
const checkProductEligibility = catchAsync(async (req, res) => {
    ...
    const clientData = pick(req.body, ['businessSegment', 'riskProfile', 'totalBalance', 'industry']);
...
const calculateEstimatedBenefits = catchAsync(async (req, res) => {
    const { productId } = req.params;
    const clientData = pick(req.body, ['currentBalance', 'currentYieldRate', 'transactionVolume']);
